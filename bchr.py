#Mizogg 08/08/2021 Random Scan for Bitcoin CASH BCH Addresses and Balance Using BITCASH Library and API Made by mizogg.co.uk
...
while True:
    ...
    bal1=key1.get_balance('bch')
    count+=1
    total+=2
    print(Fore.RED + '<== Current PrivateKey (dec) ==> ' + Fore.YELLOW + seed + Style.RESET_ALL)
    print (Fore.GREEN +  ' <================================= Bitcoin CASH Addresses Checked for Balance =================================>' + '\n' +  Style.RESET_ALL)
    ...
    print(Fore.BLUE + "---bchr2.py---" + Fore.RED + "Random Scan for Bitcoin CASH Addresses and Check Balance---------mizogg.co.uk" + Fore.BLUE + "---bchr2.py--- "  + Style.RESET_ALL + seconds_to_str())
    if float (bal) or float (bal1) > 0:
        ...
        f.write('\n=============Bitcoin CASH Address with Total Received Ammount=====================')
        f.write('\nPrivateKey (hex): ' + key.to_hex())
        f.write('\nBitcoin CASH Address Compressed : ' + caddr)
        f.write('\nBitcoin CASH Address UnCompressed :' + uaddr)
